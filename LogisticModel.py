...
def func_colinearity_rt_col(in_df_features, corr_threshold=0.6, corr_method="pearson"):
    corr_table = in_df_features.corr(method=corr_method)
    col = corr_table.columns.tolist()
    
    i = 0
    corr_over_threshold = corr_table.applymap(lambda s0: np.abs(s0)>corr_threshold)
    while i<len(col)-1:
        exclude_var = corr_over_threshold.iloc[i+1:,i].index[np.where(corr_over_threshold.iloc[i+1:,i])].tolist()
        for var in exclude_var:
            col.remove(var)
        corr_over_threshold = corr_over_threshold.loc[col, col]
        i = i+1
    return col, corr_table
...
def func_variance_inflation_factor(exog, exog_idx):
    k_vars = exog.shape[1]
    x_i = exog[:, exog_idx]
    ...
    _M = np.linalg.inv(np.dot(np.transpose(x_noti), x_noti))
    _BETA = np.dot(np.dot(_M, np.transpose(x_noti)), x_i)
    x_i_hat = (_BETA*x_noti).sum(axis=1)
    r_squared_i = np.sum(np.square(x_i_hat-np.mean(x_i)))/np.sum(np.square((x_i-np.mean(x_i))))
...
def func_multicolinearity_vif(in_df_features, with_constant=True):
    ...
    vif_table = pd.DataFrame([OrderedDict({
        "feature_name": _col,
        "VIF": func_variance_inflation_factor(exog=df.values, exog_idx=_idx),
    }) for _idx, _col in enumerate([s0 for s0 in df.columns.tolist() if s0!="constant"])])
    return vif_table


############################################################################
def func_logistic_model(
        df_X, y,
        method="newton",
        maxiter=100,
        with_intercept=True,
...
############################################################################
# stepwise
def func_logistic_model_stepwise(
        df_X, y,
        method="newton",
...
############################################################################
# forward
def func_logistic_model_forward(
        df_X, y,
        method="newton",
...
############################################################################
# backward
def func_logistic_model_backward(
        df_X, y,
        method="newton",
